...
public abstract class MarkupDataContainer extends MarkupData {
    ...
    protected void cloneChildrenInto(MetaData clone) {
    	...
    	if (markupDataContainer != null) {
    		addChildrenToClone(markupDataContainer);
    	}
    }

    // TODO: use lambdas (are all machines running on Java 8????)
    private void addChildrenToClone(MarkupDataContainer markupDataContainerClone) {
    	for (MarkupData md : children) {
//    		markupDataContainerClone.addChild(md.clone());
    	}
	}
