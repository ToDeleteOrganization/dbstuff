...
public class LocalizableSubContent extends MetaData {
    ...
    protected void copyPropertiesTo(MetaData clone) {
    	LocalizableSubContent localizableSubContent = super.convertType(clone);
    	if (localizableSubContent != null) {
    		super.copyPropertiesTo(localizableSubContent);
    		localizableSubContent.setLength(length);
    		localizableSubContent.setSourceTagContentOffset(sourceTagContentOffset);
    		localizableSubContent.setParagraphUnitId(paragraphUnitId);
    	}
    }

    @Override
    public LocalizableSubContent deepClone() {
    	LocalizableSubContent loc = new LocalizableSubContent();
    	copyPropertiesTo(loc);
    	return loc;
    }
