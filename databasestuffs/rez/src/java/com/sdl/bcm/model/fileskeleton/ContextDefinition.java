...
public class ContextDefinition extends AbstractSkeletonItem {
    ...
    protected void copyPropertiesTo(MetaData clone) {
    	ContextDefinition contextDef = convertType(clone);
    	if (contextDef != null) {
    		super.copyPropertiesTo(clone);
    		contextDef.setFormattingGroupId(formattingGroupId);
    		contextDef.setTmContext(isTmContext);
    		contextDef.setStructureContext(isStructureContext);
    		contextDef.setTypeId(typeId);
    		contextDef.setDisplayName(displayName);
    		contextDef.setDisplayCode(displayCode);
    		contextDef.setDisplayColor(displayColor);
    		contextDef.setDescription(description);
    	}
    }
