...
@JsonInclude(value = JsonInclude.Include.NON_DEFAULT) // TODO: is this necessary
public class FileSkeleton extends MetaData {
    ...
    protected void copyPropertiesTo(MetaData clone) {
    	...
    	if (fileSkeleton != null) {
    		...
    		fileSkeleton.setCommentDefinitions(Utils.deepCloneList(commentDefinitions));
    		fileSkeleton.setContextDefinitions(Utils.deepCloneList(contextDefinitions));
    		fileSkeleton.setContexts(contexts);
    		fileSkeleton.setFormattingGroups(Utils.deepCloneList(formattingGroups));
    		fileSkeleton.setTagPairDefinitions(Utils.deepCloneList(tagPairDefinitions));
    		fileSkeleton.setPlaceholderTagDefinitions(Utils.deepCloneList(placeholderTagDefinitions));
    		fileSkeleton.setStructureTagDefinitions(Utils.deepCloneList(structureTagDefinitions));
    		fileSkeleton.setTerminologyData(Utils.deepCloneList(terminologyData));
    	}
    }

    @Override
    public FileSkeleton deepClone() {
    	FileSkeleton fileSkeleton = new FileSkeleton();
    	copyPropertiesTo(fileSkeleton);
    	return fileSkeleton;
    }
